...
class MasterAuthzITestBase : public ExternalMiniClusterITestBase {
 public:
  ...
  void SetUpCluster(HarnessEnum harness) {
    ...
    string hostport = cluster_->master()->bound_http_hostport().ToString();
    string host = hostport.substr(0, hostport.find(':'));
    string kt_path;
    ASSERT_OK(
        cluster_->kdc()->CreateServiceKeytab(Substitute("HTTP/$0@KRBTEST.COM", host), &kt_path));
    PCHECK(setenv("KRB5_KTNAME", kt_path.c_str(), 1) == 0);
    // cluster_->kdc()->GetEnvVars()["KRB5_CONFIG"], kt_path;
    ASSERT_OK(cluster_->kdc()->CreateServiceKeytab("HTTP/127.0.0.1", &kt_path));
    PCHECK(setenv("KRB5_KTNAME", kt_path.c_str(), 1) == 0);
    // cluster_->ranger()->EnableKerberos(cluster_->kdc()->GetEnvVars()["KRB5_CONFIG"], kt_path);
