...
void RestCatalogPathHandlers::HandleApiTableEndpoint(const Webserver::WebRequest& req,
  ...
  string table_id;
  if (req.path_params.count("table_id") > 0) {
  ...
  if (!status.ok()) {
    RETURN_JSON_ERROR(
        jw, status.ToString(), resp->status_code, HttpStatusCode::InternalServerError);
  }
...
void RestCatalogPathHandlers::HandleGetTables(ostringstream* output,
  ...
  if (!status.ok()) {
    RETURN_JSON_ERROR(jw, status.ToString(), *status_code, HttpStatusCode::InternalServerError);
  }
  jw.StartObject();
  jw.String("tables");
  jw.StartArray();
