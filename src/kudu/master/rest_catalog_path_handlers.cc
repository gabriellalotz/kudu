...
void RestCatalogPathHandlers::HandleApiTableEndpoint(const Webserver::WebRequest& req,
  ...
  if (!l.catalog_status().ok()) {
    RETURN_JSON_ERROR(
        jw, l.catalog_status().ToString(), resp->status_code, HttpStatusCode::ServiceUnavailable);
  }
  if (!l.leader_status().ok()) {
    RETURN_JSON_ERROR(
        jw, "Master is not the leader", resp->status_code, HttpStatusCode::ServiceUnavailable);
  }
