...
// Replica rebalancing still schedules moves with --auto_rebalancing_prefer_follower_replica_moves
// enabled (default). That flag only affects follower-vs-leader preference among
// equal replica-move candidates; it is unrelated to the auto leader-rebalancer
// thread (see CheckNoLeaderMovesScheduled() elsewhere).
TEST_F(AutoRebalancerTest, TestReplicaRebalancingSchedulesMovesWithPreferFollowerTrue) {
  const bool original_prefer = FLAGS_auto_rebalancing_prefer_follower_replica_moves;
  SCOPED_CLEANUP({ FLAGS_auto_rebalancing_prefer_follower_replica_moves = original_prefer; });
...
// Same scenario as TestReplicaRebalancingSchedulesMovesWithPreferFollowerTrue
// but with --auto_rebalancing_prefer_follower_replica_moves=false: replica moves must still
// be scheduled (disabling the flag does not break replica rebalancing).
TEST_F(AutoRebalancerTest, TestReplicaRebalancingSchedulesMovesWithPreferFollowerFalse) {
  const bool original_prefer = FLAGS_auto_rebalancing_prefer_follower_replica_moves;
  SCOPED_CLEANUP({ FLAGS_auto_rebalancing_prefer_follower_replica_moves = original_prefer; });
  ...
  FLAGS_auto_rebalancing_prefer_follower_replica_moves = false;
  NO_FATALS(CheckSomeMovesScheduled());
}
