...
Status AutoRebalancerTask::CheckReplicaMovesCompleted(
  ...
  for (int i = 0; i < replica_moves->size(); ++i) {
    ...
    if (!s.ok()) {
      ...
      // Remove the replace flag of the replica.
      BulkChangeConfigRequestPB req;
      auto* modify_peer = req.add_config_changes();
      modify_peer->set_type(MODIFY_PEER);
      *modify_peer->mutable_peer()->mutable_permanent_uuid() = move.ts_uuid_from;
      ...
      string leader_uuid;
      HostPort leader_hp;
      Status removal_status = GetTabletLeader(move.tablet_uuid, &leader_uuid, &leader_hp);
      if (!removal_status.ok()) {
        LOG(WARNING) << "Removing replace flag failed: " << removal_status.message().ToString();
      } else {
        ChangeConfigResponsePB resp;
        RpcController rpc;
        req.set_dest_uuid(leader_uuid);
        ...
        vector<Sockaddr> resolved;
        removal_status = leader_hp.ResolveAddresses(&resolved);
        if (!removal_status.ok()) {
          LOG(WARNING) << "Removing replace flag failed: " << removal_status.message().ToString();
        } else {
          ConsensusServiceProxy proxy(messenger_, resolved[0], leader_hp.host());
          removal_status = proxy.BulkChangeConfig(req, &resp, &rpc);
          if (removal_status.ok() && resp.has_error()) {
            removal_status = StatusFromPB(resp.error().status());
          }
          if (!removal_status.ok()) {
            LOG(WARNING) << "Removing replace flag failed: " << removal_status.message().ToString();
          }
