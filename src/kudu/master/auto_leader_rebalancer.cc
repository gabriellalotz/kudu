...
METRIC_DEFINE_counter(server, auto_leader_rebalancer_moves_scheduled,
                      ...
                      "Number of leader transfers the auto-leader-rebalancer "
                      "attempted, across the per-table and global passes. "
                      "'Attempted' means the transfer made it past the "
                      "pre-flight applicability checks (target still known "
                      "and not in maintenance mode) and got as far as issuing "
                      "the LeaderStepDown RPC. Together with the "
                      "'_moves_completed' and '_moves_failed' counters the "
                      "invariant 'scheduled = completed + failed' holds.",
                      kudu::MetricLevel::kInfo);

METRIC_DEFINE_counter(server, auto_leader_rebalancer_moves_completed,
                      "Auto-Leader-Rebalancer Moves Completed",
                      kudu::MetricUnit::kTablets,
                      "Number of leader transfers the auto-leader-rebalancer "
                      "issued that were accepted by the target tablet server "
                      "(the LeaderStepDown RPC returned success and the "
                      "response carried no error).",
                      kudu::MetricLevel::kInfo);
...
METRIC_DEFINE_counter(server, auto_leader_rebalancer_task_errors,
                      "Auto-Leader-Rebalancer Task Errors",
                      kudu::MetricUnit::kUnits,
                      "Number of auto-leader-rebalancer rounds that returned "
                      "an unexpected error (any non-OK status from "
                      "RunLeaderRebalancer() other than this master losing "
                      "catalog-manager leadership mid-round). Suitable as the "
                      "primary alerting signal for 'the auto-leader-rebalancer "
                      "task is faltering'.",
...
AutoLeaderRebalancerTask::AutoLeaderRebalancerTask(
    ...
    : catalog_manager_(catalog_manager),
      ...
      moves_completed_(
          METRIC_auto_leader_rebalancer_moves_completed.Instantiate(metric_entity)),
      moves_failed_(
      ...
      task_errors_(
          METRIC_auto_leader_rebalancer_task_errors.Instantiate(metric_entity)),
...
Status AutoLeaderRebalancerTask::RunLeaderRebalanceForTable(
  ...
  //
  // TODO(gabriellalotz): each LeaderStepDown RPC below is issued synchronously,
  // so a round with many transfers can be slow. If we change this loop to
  // dispatch RPCs asynchronously in a follow-up, the moves_scheduled /
  // moves_completed / moves_failed counters let us verify the async version
  // isn't dropping moves.
  ...
  for (const auto& task : leader_transfer_tasks) {
    ...
    // Pre-flight checks: if the world has changed since we planned these
    // moves (the source tserver deregistered, or the destination is now in
    // maintenance mode), the transfer isn't applicable anymore. Skip it and
    // let the next round replan. These skips don't touch moves_scheduled or
    // moves_failed, since they aren't actual attempts. Keeps the invariant
    // 'scheduled = completed + failed' scoped to moves we actually tried.
...
Status AutoLeaderRebalancerTask::RunGlobalLeaderRebalance(
  ...
  for (const auto& table_info : table_infos) {
    ...
    for (const auto& task : leader_transfer_tasks) {
      ...
      // Pre-flight applicability checks: see the matching block in
      // RunLeaderRebalanceForTable() for why these skips don't touch either
      // the '_moves_scheduled' or '_moves_failed' counter.
      if (PREDICT_FALSE(TServerStatePB::MAINTENANCE_MODE ==
...
Status AutoLeaderRebalancerTask::RunLeaderRebalancer(bool* ran) {
  ...
  vector<scoped_refptr<TableInfo>> table_infos;
  {
    ...
    // Mirror the earlier check: losing catalog-manager leadership between the
    // top-of-round check and here is expected, not an error. Report OK so
    // callers can distinguish that from real failures (which do increment
    // 'auto_leader_rebalancer_task_errors').
    if (!leader_lock.first_failed_status().ok()) {
      VLOG(1) << "auto-leader-rebalancer: lost catalog-manager leadership "
                 "mid-round, skipping remainder of this round: "
              << leader_lock.first_failed_status().ToString();
      return Status::OK();
    }
  ...
  //
  // Note the gate below counts planned moves, while the
  // 'auto_leader_rebalancer_moves_scheduled' metric counts only the transfers
  // that clear the pre-flight checks. In a round where every planned move is
  // skipped in pre-flight, global_pass_skipped ticks up while moves_scheduled
  // stays flat: the two are scoped differently, not a lost move.
