...
Status CatalogManager::InitClusterId() {
  ...
  // fast lookup.
  if (s.ok()) {
    std::lock_guard l(cluster_id_lock_);
...
Status CatalogManager::CreateTable(const CreateTableRequestPB* orig_req,
  ...
  if (!rpc) {
    LOG(INFO) << "rpc is nullptr, calling CreateTableHelper with nullptr.";
    return CreateTableHelper(orig_req, resp, nullptr, std::nullopt);
  }
...
// Create a new table.
Status CatalogManager::CreateTableWithUser(const CreateTableRequestPB* orig_req,
                                           CreateTableResponsePB* resp,
                                           const optional<string>& user) {
  leader_lock_.AssertAcquiredForReading();
  LOG(INFO) << Substitute("Servicing CreateTable request from $0:\n$1",
                          user ? *user : "<default>", SecureDebugString(*orig_req));
