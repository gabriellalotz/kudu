...
class MultiMasterSpnegoTest : public RestCatalogTestBase {
 public:
  ...
  void GetLeaderMasterIndex(int* leader_idx) {
    ...
    *leader_idx = -1;
    ...
    // Find which master corresponds to the leader address
    for (int i = 0; i < cluster_->num_masters(); i++) {
      if (leader_addr.find(cluster_->mini_master(i)->bound_http_addr().ToString()) !=
          string::npos) {
        *leader_idx = i;
        return;
      }
...
TEST_F(MultiMasterSpnegoTest, TestTableOperationsWithAuthentication) {
  ...
  int leader_idx = -1;
  GetLeaderMasterIndex(&leader_idx);
