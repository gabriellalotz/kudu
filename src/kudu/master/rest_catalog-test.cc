...
class RestCatalogTest : public KuduTest {
 public:
  ...
  static int FindColumnId(string const &schema_json) {
    ...
    if (regexec(&column_id_regex, schema_json.c_str(), 2, match, 0) == 0) {
      std::string column_id = schema_json.substr(match[1].rm_so, match[1].rm_eo - match[1].rm_so);
  ...
  static string ConstructTableSchema(int column_id, bool is_there_new_column = false) {
    string column_id_str = std::to_string(column_id);
