...
class RestCatalogTest : public KuduTest {
 ...
 protected:
  ...
  std::string kTableSchema = Substitute("{\"columns\":[$0]}", columns);
  std::string kTableSchemaWithNewColumn = Substitute("{\"columns\":[$0$1]}", columns, new_column);
  std::string kTablePartitionSchema = "{\"range_schema\":{\"columns\":[{\"id\":10}]}}";
};
